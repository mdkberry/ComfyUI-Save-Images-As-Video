...
class SaveFramesToVideoFFmpeg:
    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                "images": ("IMAGE",),
                "filename_prefix": ("STRING", {"default": "VID_"}),
    ...
    def save_video(self, images, filename_prefix, fps, codec, pixel_format, output_format, 
        ...
        if not isinstance(images, torch.Tensor) or images.ndim != 4:
            ...
            log_node_error(NODE_LOG_PREFIX,error_msg); return {"ui": {"text": [error_msg]}}
        if images.shape[0] == 0:
            error_msg = "Error: No frames to process (batch_size is 0)."
            log_node_error(NODE_LOG_PREFIX,error_msg); return {"ui": {"text": [error_msg]}}

        h, w = images[0].shape[0], images[0].shape[1]
        full_output_folder, filename, counter, subfolder, _ = folder_paths.get_save_image_path(filename_prefix, self.output_dir, w, h)
        video_filename = f"{filename}_{counter:05}_.{output_format}"
        video_full_path = os.path.join(full_output_folder, video_filename)

        try:
            subprocess.run(['ffmpeg', '-version'], stdout=subprocess.PIPE, stderr=subprocess.PIPE, check=True, text=True, encoding='utf-8')
        except Exception as e:
            error_msg = f"ffmpeg version check failed: {e}. Ensure ffmpeg is installed and in PATH."
            log_node_error(NODE_LOG_PREFIX, f"ERROR: {error_msg}"); return {"ui": {"text": [error_msg]}}
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            if audio is not None and isinstance(audio, dict) and "waveform" in audio and "sample_rate" in audio:
                ...
                log_node_debug(NODE_LOG_PREFIX, f"DEBUG: Audio data. Waveform: {waveform_tensor.shape}, SR: {sample_rate}")
                if waveform_tensor.ndim == 3 and waveform_tensor.shape[0] > 0 and waveform_tensor.shape[2] > 0:
                    if waveform_tensor.shape[0] > 1: log_node_warning(NODE_LOG_PREFIX, f"Warning: Audio batch size {waveform_tensor.shape[0]}. Using first track.")
                    ...
                    if waveform_to_save.numel() > 0:
                        try:
                            ...
                            log_node_info(NODE_LOG_PREFIX, f"Audio input prepared: {temp_audio_file_for_ffmpeg}")
                        except Exception as e_asave:
                            log_node_error(NODE_LOG_PREFIX, f"Error saving temp audio: {e_asave}. Skipping audio.")
                    ...
                    else: log_node_warning(NODE_LOG_PREFIX,"Warning: Audio waveform empty. Skipping audio.")
                else: log_node_warning(NODE_LOG_PREFIX, f"Warning: Audio waveform shape {waveform_tensor.shape} unexpected. Skipping.")
            elif audio is not None: log_node_warning(NODE_LOG_PREFIX, f"Warning: Audio input not expected format. Type: {type(audio)}. Skipping.")
            ...
            if has_audio_input:
                if audio_codec == "copy": ffmpeg_cmd.extend(['-c:a', 'copy'])
                    ...
                    if audio_codec in ["aac", "mp3", "opus"]: ffmpeg_cmd.extend(['-b:a', audio_bitrate])
                ffmpeg_cmd.extend(['-shortest'])
            else: ffmpeg_cmd.extend(['-an'])
            ffmpeg_cmd.append(video_full_path)
            
            log_node_info(NODE_LOG_PREFIX, f"Executing ffmpeg: {' '.join(ffmpeg_cmd)}")
            try:
                ...
                if process.returncode != 0:
                    ...
                    log_node_error(NODE_LOG_PREFIX, err_msg); return {"ui": {"text": [f"ffmpeg error: Check console."]}}
                else:
                    log_node_success(NODE_LOG_PREFIX, f"Video saved: {video_full_path}")
                    if stdout.strip(): log_node_info(NODE_LOG_PREFIX, f"ffmpeg stdout:\n{stdout}", "WHITE")
                    if stderr.strip(): log_node_warning(NODE_LOG_PREFIX, f"ffmpeg stderr (warnings):\n{stderr}", "WHITE")
                ...
                if temp_audio_file_for_ffmpeg and os.path.exists(temp_audio_file_for_ffmpeg):
                    ...
                    except Exception as e_rem: log_node_warning(NODE_LOG_PREFIX, f"Warning: Could not remove temp audio: {e_rem}")
